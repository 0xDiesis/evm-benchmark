...
COUNTERS: dict[str, str] = {
    ...
    "commit_transactions_unique": "sum(reth_diesis_commit_proposal_transactions_unique_total)",
    "commit_duplicate_skips": "sum(reth_diesis_commit_proposal_duplicate_skipped_total)",
    "commit_capacity_skips": "sum(reth_diesis_commit_proposal_capacity_skipped_total)",
    ...
    "tx_source_scanned": "sum(reth_diesis_tx_source_scanned_total)",
    "tx_source_pulled": "sum(reth_diesis_tx_source_pulled_total)",
    "tx_source_selected_primary": "sum(reth_diesis_tx_source_selected_primary_total)",
    "tx_source_selected_fallback": "sum(reth_diesis_tx_source_selected_fallback_total)",
    "tx_source_suppressed_committed": "sum(reth_diesis_tx_source_suppressed_committed_total)",
    "tx_source_suppressed_recently_proposed": "sum(reth_diesis_tx_source_suppressed_recently_proposed_total)",
    "tx_source_skipped_gas": "sum(reth_diesis_tx_source_skipped_gas_total)",
    "tx_source_skipped_bytes": "sum(reth_diesis_tx_source_skipped_bytes_total)",
    "tx_source_skipped_partition": "sum(reth_diesis_tx_source_skipped_partition_total)",
    "tx_source_scan_limit_hit": "sum(reth_diesis_tx_source_scan_limit_hit_total)",
    "tx_source_empty_selection": "sum(reth_diesis_tx_source_empty_selection_total)",
    "tx_source_empty_selection_backoff": "sum(reth_diesis_tx_source_empty_selection_backoff_total)",
    "tx_source_empty_selection_anomalous": "sum(reth_diesis_tx_source_empty_selection_anomalous_total)",
    "tx_source_recently_proposed_cache_full": "sum(reth_diesis_tx_source_recently_proposed_cache_full_total)",
...
GAUGES: dict[str, str] = {
    ...
    "commit_last_unique_transactions": "sum(reth_diesis_commit_proposal_last_unique_transactions)",
    "commit_last_duplicate_skips": "sum(reth_diesis_commit_proposal_last_duplicate_skips)",
    "commit_last_capacity_skips": "sum(reth_diesis_commit_proposal_last_capacity_skips)",
    "commit_last_skip_ratio": "avg(reth_diesis_commit_proposal_last_skip_ratio)",
    "tx_source_pool_pending": "sum(reth_diesis_tx_source_pool_pending)",
    "tx_source_last_pulled": "sum(reth_diesis_tx_source_last_pulled)",
    "tx_source_last_scanned": "sum(reth_diesis_tx_source_last_scanned)",
    "tx_source_proposal_utilization": "avg(reth_diesis_tx_source_proposal_utilization)",
...
def print_summary(args: argparse.Namespace) -> int:
    ...
    important = [
        ...
        ("Tx source scanned", counters.get("tx_source_scanned", {}).get("delta")),
        ("Tx source pulled", counters.get("tx_source_pulled", {}).get("delta")),
        ("Tx source primary selected", counters.get("tx_source_selected_primary", {}).get("delta")),
        ("Tx source fallback selected", counters.get("tx_source_selected_fallback", {}).get("delta")),
        (
            "Tx source suppressed committed",
            counters.get("tx_source_suppressed_committed", {}).get("delta"),
        ),
        (
            "Tx source suppressed recent",
            counters.get("tx_source_suppressed_recently_proposed", {}).get("delta"),
        ),
        ("Tx source skipped gas", counters.get("tx_source_skipped_gas", {}).get("delta")),
        ("Tx source skipped bytes", counters.get("tx_source_skipped_bytes", {}).get("delta")),
        (
            "Tx source skipped partition",
            counters.get("tx_source_skipped_partition", {}).get("delta"),
        ),
        ("Tx source scan limit hits", counters.get("tx_source_scan_limit_hit", {}).get("delta")),
        ("Tx source empty selections", counters.get("tx_source_empty_selection", {}).get("delta")),
        (
            "Tx source empty backoff",
            counters.get("tx_source_empty_selection_backoff", {}).get("delta"),
        ),
        (
            "Tx source empty anomalous",
            counters.get("tx_source_empty_selection_anomalous", {}).get("delta"),
        ),
        ("Commit unique txs", counters.get("commit_transactions_unique", {}).get("delta")),
        ("Commit duplicate skips", counters.get("commit_duplicate_skips", {}).get("delta")),
        ("Commit capacity skips", counters.get("commit_capacity_skips", {}).get("delta")),
        ("Commit last skip ratio", gauges.get("commit_last_skip_ratio", {}).get("after")),
        (
            "Tx source proposal utilization",
            gauges.get("tx_source_proposal_utilization", {}).get("after"),
        ),
